...
export const Clients = () => {
  ...
  const fetchClients = async (isLoadingMore = false) => {
    if (isLoadingMore) {
      setLoadingMore(true)
    } else {
      setLoading(true)
    ...
    try {
      const result = await getClients(20, lastDoc || undefined)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Gerencie todos os clientes da empresa</p>
        </div>
      ...
      <div className="bg-slate-800 shadow-sm border border-slate-700 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="mt-4 text-lg font-medium text-white">Nenhum cliente encontrado</h3>
        ...
        {/* Load More Button */}
        {!loading && hasMore && filteredClients.length > 0 && (
        ...
        >
          <div className="space-y-4">
            <p className="text-slate-300">
              Tem certeza que deseja excluir o cliente <strong className="text-white">{deleteConfirm.name}</strong>?<br />
